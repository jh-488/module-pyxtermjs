...
import { TrzszFileReader } from "./comm";
...
export class TrzszFilter {
   ...
   */
  public async uploadFiles(items: string[] | DataTransferItemList | FileList) {
    if (this.uploadFilesList || this.isTransferringFiles()) {
      throw new Error("The previous upload has not been completed yet");
    }
    if (items instanceof DataTransferItemList || items instanceof FileList) {
      this.uploadFilesList = await parseDataTransferItemList(items as DataTransferItemList | FileList);
